dic_num = {'one': 1, 'two': 2, 'three': 3, 'four': 4, 'five': 5, 'six': 6, 'seven': 7, 'eight': 8, 'nine': 9, 'ten': 10,
           'eleven': 11, 'twelve': 12, 'thirteen': 13, 'fourteen': 14, 'fifteen': 15, 'sixteen': 16, 'seventeen': 17,
           'eighteen': 18, 'nineteen': 19, 'twenty': 20}
# data_input = 'five thirteen two eleven seventeen two one thirteen ten four eight five nineteen'
data_input = input('Input your dic:\n').split()
print(data_input)

print('Task 1:\n')
data_input = [dic_num[i] for i in data_input]
print(data_input)

print('Task 2 and 3:\n')
data_input = list(set(data_input))
print(data_input)

print('Task 4:\n')
for i in range(len(data_input)-1):
    if i % 2 == 0:
        print(data_input[i]*data_input[i+1], end=' ')
    else:
        print(data_input[i] + data_input[i + 1], end=' ')
print()

print('Task 5:\n')
print(sum([i for i in data_input if i % 2 != 0]))